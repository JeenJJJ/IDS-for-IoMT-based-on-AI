"""
Hybride 7 — CNN + LSTM + LightGBM
Deep + Deep + Boosting
CNN extrait features spatiales → LSTM features temporelles → LightGBM classifie
"""
import numpy as np
...
def build_model(input_dim, n_classes):
    ...
    # LSTM attend (batch, timesteps, features) — ici timesteps=input_dim, features=32
    x = LSTM(64, return_sequences=False, name='lstm_out')(x)
    x = Dropout(0.3)(x)
    out = Dense(n_classes, activation='softmax')(x)

    model = Model(inputs=inp, outputs=out)
    extractor = Model(inputs=inp, outputs=model.get_layer('lstm_out').output)
...
def run(X_tr, y_tr, X_te, label):
    ...
    model, extractor = build_model(X_tr.shape[1], n_classes)
